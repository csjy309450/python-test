...
tf.app.flags.DEFINE_string('data_dir', '/home/yangzheng/myPrograms/Libs_test/python-test/tensorflow_test/saifar10/cifar10_data',
...
def inference(images):
  ...
  with tf.variable_scope('local3') as scope:
    ...
    weights = _variable_with_weight_decay('weights', shape=[dim, 32],
  ...
  with tf.variable_scope('local4') as scope:
    weights = _variable_with_weight_decay('weights', shape=[32, 48],
                                          stddev=0.04, wd=0.004)
...
def predict_inference(images):
  ...
  with tf.variable_scope('local3') as scope:
    # Move everything into depth so we can perform a single matrix multiply.
    dim0 = images.get_shape()[0].value
    reshape = tf.reshape(pool2, [dim0, -1])
    dim = reshape.get_shape()[1].value
    weights = _variable_with_weight_decay('weights', shape=[dim, 32],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [32], tf.constant_initializer(0.1))
...
def train(total_loss, global_step):
  ...
  # apply_gradient_op = opt.minimize(total_loss, global_step)
