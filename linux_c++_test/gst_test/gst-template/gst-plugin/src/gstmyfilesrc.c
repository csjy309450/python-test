...
static int
gst_my_file_src_open (const gchar * filename, int flags, int mode)
{
#ifdef G_OS_WIN32
...
static void gst_my_file_src_finalize (GObject * object);

static void gst_my_file_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_my_file_src_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_my_file_src_start (GstBaseSrc * basesrc);
static gboolean gst_my_file_src_stop (GstBaseSrc * basesrc);

static gboolean gst_my_file_src_is_seekable (GstBaseSrc * src);
static gboolean gst_my_file_src_get_size (GstBaseSrc * src, guint64 * size);
static GstFlowReturn gst_my_file_src_fill (GstBaseSrc * src, guint64 offset,
    guint length, GstBuffer * buf);

static void gst_my_file_src_uri_handler_init (gpointer g_iface,
    gpointer iface_data);

#define _do_init \
  G_IMPLEMENT_INTERFACE (GST_TYPE_URI_HANDLER, gst_my_file_src_uri_handler_init); \
  GST_DEBUG_CATEGORY_INIT (gst_file_src_debug, "filesrc", 0, "filesrc element");
#define gst_file_src_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstMyFileSrc, gst_file_src, GST_TYPE_BASE_SRC, _do_init);

static void
gst_file_src_class_init (GstMyFileSrcClass * klass)
{
  ...
  gobject_class->set_property = gst_my_file_src_set_property;
  gobject_class->get_property = gst_my_file_src_get_property;
  ...
  gobject_class->finalize = gst_my_file_src_finalize;
  ...
  gstbasesrc_class->start = GST_DEBUG_FUNCPTR (gst_my_file_src_start);
  gstbasesrc_class->stop = GST_DEBUG_FUNCPTR (gst_my_file_src_stop);
  gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR (gst_my_file_src_is_seekable);
  gstbasesrc_class->get_size = GST_DEBUG_FUNCPTR (gst_my_file_src_get_size);
  gstbasesrc_class->fill = GST_DEBUG_FUNCPTR (gst_my_file_src_fill);
...
static void
gst_file_src_init (GstMyFileSrc * src)
{
  src->filename = NULL;
  src->fd = 0;
  src->uri = NULL;
...
static void
gst_my_file_src_finalize (GObject * object)
{
  GstMyFileSrc *src;

  src = GST_MY_FILE_SRC (object);
...
static gboolean
gst_file_src_set_location (GstMyFileSrc * src, const gchar * location)
{
  GstState state;
...
static void
gst_my_file_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMyFileSrc *src;

  g_return_if_fail (GST_IS_MY_FILE_SRC (object));
...
static void
gst_my_file_src_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstMyFileSrc *src;
...
gst_my_file_src_fill (GstBaseSrc * basesrc, guint64 offset, guint length,
    GstBuffer * buf)
{
  GstMyFileSrc *src;
  ...
  src = GST_MY_FILE_SRC_CAST (basesrc);
  ...
  while (to_read > 0) {
	...
	// test
	guint8 * t_read_data = g_new(guint8, bytes_read+1);
	memset(t_read_data,0,bytes_read+1);
	memcpy(t_read_data,data,bytes_read);
	g_print("new data : %s\n",t_read_data);
...
static gboolean
gst_my_file_src_is_seekable (GstBaseSrc * basesrc)
{
  GstMyFileSrc *src = GST_MY_FILE_SRC (basesrc);

  return src->seekable;
}

static gboolean
gst_my_file_src_get_size (GstBaseSrc * basesrc, guint64 * size)
{
  struct stat stat_results;
  GstMyFileSrc *src;

  src = GST_MY_FILE_SRC (basesrc);
...
static gboolean
gst_my_file_src_start (GstBaseSrc * basesrc)
{
  GstMyFileSrc *src = GST_MY_FILE_SRC (basesrc);
  struct stat stat_results;
  ...
  /* open the file */
  src->fd = gst_my_file_src_open (src->filename, O_RDONLY | O_BINARY, 0);
...
static gboolean
gst_my_file_src_stop (GstBaseSrc * basesrc)
{
  GstMyFileSrc *src = GST_MY_FILE_SRC (basesrc);
...
{
  GstMyFileSrc *src = GST_MY_FILE_SRC (handler);
...
static void
gst_my_file_src_uri_handler_init (gpointer g_iface, gpointer iface_data)
{
