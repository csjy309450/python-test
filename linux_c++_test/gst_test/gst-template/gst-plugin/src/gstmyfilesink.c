...
buffer_mode_get_type (void)
{
  ...
  if (!buffer_mode_type) {
    buffer_mode_type =
        g_enum_register_static ("GstMyFileSinkBufferMode", buffer_mode);
  }
  return buffer_mode_type;
}

GST_DEBUG_CATEGORY_STATIC (gst_my_file_sink_debug);
#define GST_CAT_DEFAULT gst_my_file_sink_debug
...
static void gst_my_file_sink_dispose (GObject * object);

static void gst_my_file_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_my_file_sink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_my_file_sink_open_file (GstMyFileSink * sink);
static void gst_my_file_sink_close_file (GstMyFileSink * sink);

static gboolean gst_my_file_sink_start (GstBaseSink * sink);
static gboolean gst_my_file_sink_stop (GstBaseSink * sink);
static gboolean gst_my_file_sink_event (GstBaseSink * sink, GstEvent * event);
static GstFlowReturn gst_my_file_sink_render (GstBaseSink * sink,
    GstBuffer * buffer);

static gboolean gst_my_file_sink_do_seek (GstMyFileSink * filesink,
    guint64 new_offset);
static gboolean gst_my_file_sink_get_current_offset (GstMyFileSink * filesink,
    guint64 * p_pos);

static gboolean gst_my_file_sink_query (GstBaseSink * bsink, GstQuery * query);

static void gst_my_file_sink_uri_handler_init (gpointer g_iface,
    gpointer iface_data);

#define _do_init \
  G_IMPLEMENT_INTERFACE (GST_TYPE_URI_HANDLER, gst_my_file_sink_uri_handler_init); \
  GST_DEBUG_CATEGORY_INIT (gst_my_file_sink_debug, "myfilesink", 0, "mydfilesink element");
#define gst_my_file_sink_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstMyFileSink, gst_my_file_sink, GST_TYPE_BASE_SINK,
    _do_init);

static void
gst_my_file_sink_class_init (GstMyFileSinkClass * klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GstElementClass *gstelement_class = GST_ELEMENT_CLASS (klass);
  GstBaseSinkClass *gstbasesink_class = GST_BASE_SINK_CLASS (klass);

  gobject_class->dispose = gst_my_file_sink_dispose;

  gobject_class->set_property = gst_my_file_sink_set_property;
  gobject_class->get_property = gst_my_file_sink_get_property;
  ...
  gstbasesink_class->start = GST_DEBUG_FUNCPTR (gst_my_file_sink_start);
  gstbasesink_class->stop = GST_DEBUG_FUNCPTR (gst_my_file_sink_stop);
  gstbasesink_class->query = GST_DEBUG_FUNCPTR (gst_my_file_sink_query);
  gstbasesink_class->render = GST_DEBUG_FUNCPTR (gst_my_file_sink_render);
  gstbasesink_class->event = GST_DEBUG_FUNCPTR (gst_my_file_sink_event);
...
static void
gst_my_file_sink_init (GstMyFileSink * filesink)
{
...
static void
gst_my_file_sink_dispose (GObject * object)
{
  GstMyFileSink *sink = GST_MY_FILE_SINK (object);
...
static gboolean
gst_my_file_sink_set_location (GstMyFileSink * sink, const gchar * location,
    GError ** error)
{
  if (sink->file)
    goto was_open;
...
static void
gst_my_file_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMyFileSink *sink = GST_MY_FILE_SINK (object);
  ...
  switch (prop_id) {
    case PROP_LOCATION:
      gst_my_file_sink_set_location (sink, g_value_get_string (value), NULL);
      break;
...
static void
gst_my_file_sink_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstMyFileSink *sink = GST_MY_FILE_SINK (object);
...
static gboolean
gst_my_file_sink_open_file (GstMyFileSink * sink)
{
  gint mode;
  ...
  sink->seekable = gst_my_file_sink_do_seek (sink, 0);
...
static void
gst_my_file_sink_close_file (GstMyFileSink * sink)
{
  if (sink->file) {
...
static gboolean
gst_my_file_sink_query (GstBaseSink * bsink, GstQuery * query)
{
  gboolean res;
  GstMyFileSink *self;
  GstFormat format;

  self = GST_MY_FILE_SINK (bsink);
...
static gboolean
gst_my_file_sink_do_seek (GstMyFileSink * filesink, guint64 new_offset)
{
  ...
  gst_my_file_sink_get_current_offset (filesink, &filesink->current_pos);
...
static gboolean
gst_my_file_sink_event (GstBaseSink * sink, GstEvent * event)
{
  GstEventType type;
  GstMyFileSink *filesink;

  filesink = GST_MY_FILE_SINK (sink);
    ...
    case GST_EVENT_SEGMENT:
    {
      ...
      if (segment->format == GST_FORMAT_BYTES) {
        ...
        if (filesink->current_pos != segment->start) {
          ...
          if (!gst_my_file_sink_do_seek (filesink, (guint64) segment->start))
            goto seek_failed;
        } else {
...
static gboolean
gst_my_file_sink_get_current_offset (GstMyFileSink * filesink, guint64 * p_pos)
{
  off_t ret = -1;
...
gst_my_file_sink_render (GstBaseSink * sink, GstBuffer * buffer)
{
  GstMyFileSink *filesink;
  GstMapInfo info;
...
static gboolean
gst_my_file_sink_start (GstBaseSink * basesink)
{
  return gst_my_file_sink_open_file (GST_MY_FILE_SINK (basesink));
}

static gboolean
gst_my_file_sink_stop (GstBaseSink * basesink)
{
  gst_my_file_sink_close_file (GST_MY_FILE_SINK (basesink));
  return TRUE;
}
...
static GstURIType
gst_my_file_sink_uri_get_type (GType type)
{
  return GST_URI_SINK;
}

static const gchar *const *
gst_my_file_sink_uri_get_protocols (GType type)
{
...
static gchar *
gst_my_file_sink_uri_get_uri (GstURIHandler * handler)
{
  GstMyFileSink *sink = GST_MY_FILE_SINK (handler);
...
static gboolean
gst_my_file_sink_uri_set_uri (GstURIHandler * handler, const gchar * uri,
    GError ** error)
{
  gchar *location;
  gboolean ret;
  ...
  if (g_str_has_prefix (uri, "file://localhost/")) {
    ...
    gst_my_file_sink_set_location (sink, NULL, NULL);
    return TRUE;
  } else {
  ...
  ret = gst_my_file_sink_set_location (sink, location, error);
  g_free (location);
...
static void
gst_my_file_sink_uri_handler_init (gpointer g_iface, gpointer iface_data)
{
  GstURIHandlerInterface *iface = (GstURIHandlerInterface *) g_iface;

  iface->get_type = gst_my_file_sink_uri_get_type;
  iface->get_protocols = gst_my_file_sink_uri_get_protocols;
  iface->get_uri = gst_my_file_sink_uri_get_uri;
  iface->set_uri = gst_my_file_sink_uri_set_uri;
}
